'use client';
// A heavy-duty modal for onboarding flows and tutorials.
// It uses getBoundingClientRect() to smartly anchor itself to specific UI elements.
// We lock body scrolling while it's open so the user doesn't accidentally wreck the positioning.
...
import { easeFast, easeSmooth, reducedMotionTransition, springSmooth } from '../utils/animations';
...
export function TutorialModal({
  ...
  const titleId = `ism-tutorial-title-${useId()}`;
  ...
  // Wipe internal state clean when the modal closes so we're fresh for the next run.
  useEffect(() => {
    if (!isOpen) {
  ...
  }, [isOpen, onOpenChange, portalRoot]);
  ...
  // Lock down user scrolling while the modal is open, but still allow programmatic smooth scrolling under the hood.
  useEffect(() => {
    if (!isOpen) return;
  ...
  if (targetRect) {
    ...
    // Fallback to a standard centered modal if no target element is provided.
  ...
  const modalOverlay = (
    <div className="fixed inset-0 z-[200] pointer-events-none flex flex-col justify-center p-4">
      {/* Dim the background to draw focus if we aren't pointing at a specific target */}
      ...
      {!isTargetPending && (
        <motion.div
          ...
          initial={
            shouldReduceMotion
              ? { opacity: 0 }
              : { opacity: 0, y: targetRect ? 8 : 20, scale: 0.95 }
          }
          animate={shouldReduceMotion ? { opacity: 1 } : { opacity: 1, y: 0, scale: 1 }}
          exit={shouldReduceMotion ? { opacity: 0 } : { opacity: 0, scale: 0.95 }}
          transition={shouldReduceMotion ? reducedMotionTransition : springSmooth}
        ...
        >
          {targetRect && arrowDir && (
            <div
              className="absolute w-4 h-4 rounded-[3px] bg-bg-surface rotate-45 pointer-events-none z-[201]"
              style={{
          ...
          <AnimatePresence>
            {shouldHideHeader && (
              <motion.button
                ...
                onClick={() => onOpenChange(false)}
                className="absolute top-3 right-3 p-1.5 rounded-md hover:bg-bg-elevated text-text-muted hover:text-text-primary transition-colors cursor-pointer z-10 bg-bg-surface shadow-sm border border-border-subtle"
                aria-label="Close tutorial"
              >
                <X size={16} />
              </motion.button>
                ...
                {!step.hideImage && step.image && (
                  ...
                  >
                    ...
                    <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center pointer-events-none rounded-lg">
                      <div className="bg-bg-surface text-text-primary px-3 py-1.5 rounded-full flex items-center gap-2 text-sm font-medium shadow-lg">
  ...
  const zoomOverlay = enlargedImage && (
    <motion.div
      ...
      className="fixed inset-0 z-[9999] bg-black/85 flex items-center justify-center p-6 md:p-12 cursor-zoom-out pointer-events-auto"
    >
      <button
        ...
        <AnimatePresence>
          {isOpen && step && (
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              transition={shouldReduceMotion ? reducedMotionTransition : easeFast}
